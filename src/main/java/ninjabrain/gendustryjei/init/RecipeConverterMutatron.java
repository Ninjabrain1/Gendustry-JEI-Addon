		...
		for (IBeeMutation mutation : mutations) {
			if (mutation.getTemplate() != null && mutation.getTemplate().length != 0) {
				...
				if (include) {
					...
					IIndividual queen = BeeManager.beeRoot.templateAsIndividual(mutation.getTemplate());
					...
					ItemStack output = BeeManager.beeRoot.getMemberStack(queen, EnumBeeType.QUEEN);
					// TODO add recipes to drones and princesses too
					wrapperList.add(new WrapperMutatron(input1, input2, output));
				}
			}
		}
...
//		System.out.println(System.nanoTime()-startTime);
	...
	public void processRecipeStatement(RecipeStatement rs) {
		if (rs instanceof RsMutation) {
			...
			System.out.println("MUTATION: " + input1 + input2 + output);
//			String inputRegistryName = getRegistryName(input);
//			wrapperList.add(new WrapperDNA(new ItemStack(Item.getByNameOrId(inputRegistryName)), rsMutation.mb()));
		}
	}
